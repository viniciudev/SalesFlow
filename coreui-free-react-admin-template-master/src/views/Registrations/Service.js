...
class FormService extends Component {
  ...
  addDetail = () => {
    ...
    if (newDetail.trim() !== "") {
      // Alterado: Adiciona um objeto ao invés de string
      const updatedDetails = [
        ...modelService.details,
        { description: newDetail.trim() },
      ];
      this.setState({
  ...
  save = async () => {
    if (this.validate() === 0) {
      ...
      let data = {
        ...
        details: modelService.details, // Já está como array de objetos
      };
  ...
  componentWillMount() {
    PubSub.subscribe("edit-service", (topic, service) => {
      // Garante que os detalhes sejam objetos {description: ""}
            ...
            <FormGroup>
              ...
              <div className="details-container">
                {modelService.details.map((detail, index) => (
                  ...
                  >
                    {detail.description}{" "}
                    {/* Alterado: exibe a propriedade description */}
                    <FaTimes
...
export default class Service extends Component {
  ...
  consultAll = async () => {
    try {
      ...
      const { data } = resp;
      if (data) {
        // Converte detalhes antigos (strings) para objetos se necessário
        const results = data.results.map((service) => ({
          ...service,
