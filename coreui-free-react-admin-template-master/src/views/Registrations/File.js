    ...
    if (this.validate() == 0) {
      ...
      if (files.length == 0) {
        ...
        const descriptionFiles = {
          ...
          groupItems: modelFile.groupItems,
        };
        ...
        await axios.post(URL_DescriptionFiles, formData).then((resp) => {
          const { data } = resp;
          if (data) {
              ...
              <Container className="container-md">
                <div className="row">
                  <Col xs="6" md="9">
                    <label
                      ...
                      className="btn btn-info btn-sucess"
                    >
                      <input
                        ref="inputImage"
                        id="inputImage"
                      ...
                      >
                        <em className="fa fa-upload"></em> upload
                      </span>
                    </label>
                  </Col>
                </div>
                <div className="row">
                  <div className="mt-3">
                    {filesBlob.length > 0 ? (
                      <div>
                        <CreateImageItem filesBlob={filesBlob} />
                      </div>
                    ) : (
                      <div>
                        <small>Nenhuma Imagem Importada!.</small>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <CardBody>
              ...
              <div className="float-left">
                <Button
                  size="sm"
                  color="primary"
                  onClick={(e) => this.NewsFiles()}
                >
                  <i className="fa fa-plus-square-o"></i> Novo Cadastro
                </Button>{" "}
    ...
    return (
      <div>
        <Row>
          {filesBlob.map((f) => (
            <Col md={3}>
              <img className="img-fluid mb-2" src={f} alt="Item" />
            </Col>
          ))}
        </Row>
      </div>
...
class ListFile extends Component {
  ...
  consult = async () => {
    ...
    await axios
      ...
      .then((resp) => {
        const { data } = resp;
        if (data) this.setState({ formFiles: data });
      });
  };
  ...
  delete = (e, idDescriptionFiles) => {
    swal("Deseja Realmente Excluir o Arquivo?", {
      ...
      if (resp) {
        axios
          .delete(`${URL_DescriptionFiles}/${idDescriptionFiles}`)
          .then((resp) => {
            const { data } = resp;
            if (data) {
              this.consult();
            ...
            <form name="formFilter">
              <div className="form-row">
                ...
                <div className="col-md-1">
                  <Button color="primary" onClick={(e) => this.consult()}>
                    Pesquisar
                  </Button>
                </div>
              </div>
                    ...
                    <div className="d-flex align-items-left">
                      ...
                      <div className="ml-auto">
                        <Pagination
                          ...
                          onChange={(pageNumber) =>
                            this.query(pageSize, pageNumber)
                          }
                          itemClass="page-item"
                          linkClass="page-link"
