...
public class DanfeNfceDocument : IDocument
{
	...
	private byte[] ImagemQrCode()
	{
		...
		try
		{
			var qrCodeText = ObtemUrlQrCode();

			// Método alternativo usando QRCoder em vez de SkiaSharp.QrCode
			using (var memoryStream = new MemoryStream())
			{
				// Opção 1: Usar QRCoder (mais confiável)
				using (var qrGenerator = new QRCoder.QRCodeGenerator())
				{
					var qrCodeData = qrGenerator.CreateQrCode(qrCodeText, QRCoder.QRCodeGenerator.ECCLevel.Q);
					using (var qrCode = new QRCoder.PngByteQRCode(qrCodeData))
					{
						var qrCodeBytes = qrCode.GetGraphic(20);
						memoryStream.Write(qrCodeBytes, 0, qrCodeBytes.Length);
		...
		catch (Exception ex)
		{
			...
			// Retorna um QR Code vazio ou imagem de erro
