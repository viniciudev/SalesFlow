...
private readonly IPurchaseItemRepository _purchaseItemRepository;
        public PurchaseService(IGenericRepository<Purchase> repository, ContextBase dbContext,
            ...
            IPurchaseItemRepository purchaseItemRepository) : base(repository)
        {
            ...
            using (var transaction = await repository.CreateTransactionAsync())
            {
                try
                {
                    ...
                    Purchase compra = new Purchase
                    {
                        ...
                        Observacao = purchaseDto.Observacao
                    };
                    ...
                    await GenerateFinancial(purchaseDto.FormPayment, compra.Id, purchaseDto.IdCompany, null,
                        purchaseDto.BankAccountId, purchaseDto.Troco);
            ...
            using (var transaction = await repository.CreateTransactionAsync())
            {
                try
                {
                    ...
                    foreach (var item in existingItems)
                    {
                        ...
                        Stock stock =
                            await _stockService.GetByReferenceIdAsync(item.Id, compra.IdCompany, StockType.entry);
        ...
        public async Task DeleteCompraAsync(int id)
        {
            using (var transaction = await repository.CreateTransactionAsync())
            {
                try
                {
                    var Purchase = await (repository as IPurchaseRepository).GetByIdWithItems(id);
                    //deletar financeiro

                    foreach (var item in Purchase.Financials)
                    {
                        if (item.FinancialPaymentMethods != null)
                        {
                            foreach (var paymentMethod in item.FinancialPaymentMethods)
                            {
                    ...
                    //deletar intens compras
                    foreach (var item in Purchase.PurchaseItems)
                    {
                        //retirar do estoque
                        var stock = await _stockService.GetByReferenceIdAsync(item.Id, Purchase.IdCompany,
                            StockType.entry);
                        if (stock != null)
                        {
                            await _stockService.Create(new Stock
                            {
                                ...
                                Date = DateTime.Now,
                                IdProduct = (int)item.ProdutoId,
                                Reason = $"Cancelamento de compra: {Purchase.Id}",
                                Type = StockType.exit,
                                ReferenceId = null
                            });
                        }
