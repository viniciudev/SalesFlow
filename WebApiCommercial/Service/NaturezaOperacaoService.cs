...
namespace Service
{
    public class NaturezaOperacaoService : BaseService<NaturezaOperacao>, INaturezaOperacaoService
    {
        ...
        public async Task<int> CreateAsync(NaturezaOperacaoCreateRequest request)
        {
            ...
            {
                CompanyId = request.CompanyId, Descricao = request.Descricao, Cfop = request.Cfop,
                TipoDocumento = request.TipoDocumento, Finalidade = request.Finalidade,
                ConsumidorFinal = request.ConsumidorFinal, MovimentaEstoque = request.MovimentaEstoque,
                Ativo = request.Ativo, PermiteTributacaoPorProduto = request.PermiteTributacaoPorProduto,
        ...
        public async Task UpdateAsync(int id, NaturezaOperacaoUpdateRequest request)
        {
            ...
            if (request.RegrasFiscais != null && request.RegrasFiscais.Any())
            {
                existing.RegrasFiscais = MapRegrasFiscais(request.RegrasFiscais);
                ValidateRegrasFiscaisUnicidade(existing.RegrasFiscais);
            }
        ...
        {
            var natureza = await base.GetByIdAsync(naturezaOperacaoId);
            ...
            natureza.RegrasFiscais = novasRegras;
            await base.Alter(natureza);
        }
        ...
        public async Task ClonarMatrizAsync(int origemId, int destinoId)
        {
            ...
            var destino = await base.GetByIdAsync(destinoId);
            ...
            {
                Id = 0, NaturezaOperacaoId = destinoId,
                SituacaoTributariaId = r.SituacaoTributariaId, Destino = r.Destino, Cfop = r.Cfop,
                ConfiguracaoTributaria = CloneConfiguracao(r.ConfiguracaoTributaria)
            }).ToList();
            destino.RegrasFiscais = regrasClonadas;
            await base.Alter(destino);
        }
        ...
        private ConfiguracaoTributaria CloneConfiguracao(ConfiguracaoTributaria origem)
        {
            ...
            {
                AplicarICMS = origem.AplicarICMS, CstICMS = origem.CstICMS, CsosnICMS = origem.CsosnICMS,
                AliquotaICMS = origem.AliquotaICMS, ReduzirBaseICMS = origem.ReduzirBaseICMS,
                AplicarIPI = origem.AplicarIPI, CstIPI = origem.CstIPI, AliquotaIPI = origem.AliquotaIPI,
                AplicarPIS = origem.AplicarPIS, CstPIS = origem.CstPIS, AliquotaPIS = origem.AliquotaPIS,
                AplicarCOFINS = origem.AplicarCOFINS, CstCOFINS = origem.CstCOFINS, AliquotaCOFINS = origem.AliquotaCOFINS,
                AplicarISSQN = origem.AplicarISSQN, AliquotaISSQN = origem.AliquotaISSQN,
                AplicarIBS = origem.AplicarIBS, CstIBS = origem.CstIBS, AliquotaIBS = origem.AliquotaIBS,
                AplicarCBS = origem.AplicarCBS, CstCBS = origem.CstCBS, AliquotaCBS = origem.AliquotaCBS,
                AplicarIS = origem.AplicarIS, AliquotaIS = origem.AliquotaIS, cClassTrib = origem.cClassTrib,
            };
        }
        ...
        private NaturezaOperacaoResponse MapToResponse(NaturezaOperacao e)
        {
            ...
            {
                Id = e.Id, Descricao = e.Descricao, Cfop = e.Cfop,
                TipoDocumento = e.TipoDocumento, Finalidade = e.Finalidade,
                ConsumidorFinal = e.ConsumidorFinal, MovimentaEstoque = e.MovimentaEstoque,
                Ativo = e.Ativo, PermiteTributacaoPorProduto = e.PermiteTributacaoPorProduto,
                ConfiguracaoTributaria = e.ConfiguracaoTributaria,
                RegrasFiscais = e.RegrasFiscais?.Select(r => new RegraFiscalDto
                {
                    Id = r.Id, SituacaoTributariaId = r.SituacaoTributariaId,
                    SituacaoTributariaCodigo = r.SituacaoTributaria?.Codigo,
                    SituacaoTributariaDescricao = r.SituacaoTributaria?.Descricao,
                    Destino = r.Destino, Cfop = r.Cfop,
        ...
        private List<RegraFiscal> MapRegrasFiscais(List<RegraFiscalDto> dtos)
        {
            ...
            {
                Id = dto.Id ?? 0, SituacaoTributariaId = dto.SituacaoTributariaId,
                Destino = dto.Destino, Cfop = dto.Cfop,
